  ...
  const fetchData = useCallback(async (signal?: { cancelled: boolean }) => {
    ...
    try {
      ...
      const mappedProducts: AppProduct[] = (prodsResponse.data as ProductWithDetails[] || []).map((p) => ({
        ...
        wholesaleMinQuantity: p.wholesale_min_quantity ? Number(p.wholesale_min_quantity) : 12,
    ...
    try {
      ...
      const { error: productError } = await supabase
        ...
        .update({
          ...
          wholesale_price: form.wholesalePrice ? Number(form.wholesalePrice) : null,
          wholesale_min_quantity: form.wholesaleMinQuantity ? Number(form.wholesaleMinQuantity) : 12,
