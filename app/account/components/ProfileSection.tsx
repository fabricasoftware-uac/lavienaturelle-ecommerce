...
import { SessionUser } from "../perfil/actions"

interface ProfileSectionProps {
  user: SessionUser
  onUpdate: (data: { name: string; phone: string; document_number: string }) => Promise<{ success: boolean; error?: string }>
  onChangePassword: (password: string) => Promise<{ success: boolean; error?: string }>
}

export function ProfileSection({ user, onUpdate, onChangePassword }: ProfileSectionProps) {
  ...
  const formatMemberSince = (_dateString?: string) => {
    return "Reciente"
  }
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      const result = await onChangePassword(passwordData.newPassword)
      if (result.success) {
        toast({
      ...
      <div className="flex max-w-2xl flex-col md:flex-row items-center gap-8 bg-white border border-stone-100 p-10 rounded-[40px] shadow-sm">
        <div className="text-center md:text-left space-y-1">
          ...
          <div className="flex items-center justify-center md:justify-start gap-3 mt-4">
            <span className="text-xs text-stone-400 font-medium">Miembro desde {formatMemberSince()}</span>
          </div>
        </div>
      </div>
