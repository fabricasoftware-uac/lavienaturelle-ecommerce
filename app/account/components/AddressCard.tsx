...
export function AddressCard({ addr, onEdit, onDelete }: AddressCardProps) {
  return (
    <div key={addr.id} className={cn(
      "group relative bg-white border rounded-4xl p-8 transition-all duration-300 h-full",
      addr.is_default 
      ...
      <div className="flex flex-col h-full">
         ...
         <p className="text-sm font-bold text-stone-400 uppercase tracking-widest mb-4">{addr.full_name}</p>
         
         <div className="space-y-1 mb-8">
            <p className="text-stone-600 font-medium leading-relaxed">{addr.address_line1}</p>
            <p className="text-stone-500 text-sm">{addr.city}, {addr.state}</p>
         ...
         <div className="mt-auto flex items-center gap-3 pt-6 border-t border-stone-50">
            <Button 
              ...
              size="sm" 
              onClick={onEdit}
              className="h-10 px-4 rounded-xl text-stone-500 hover:text-stone-900 font-bold text-xs"
            >
            ...
            <Button 
              ...
              size="sm" 
              onClick={onDelete}
              className="h-10 px-4 rounded-xl text-stone-400 hover:text-red-600 font-bold text-xs"
            >
